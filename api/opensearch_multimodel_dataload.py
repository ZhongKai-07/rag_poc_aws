...
def get_opensearch_client() -> Any:
    ...
    try:
        ...
        logger.info(f"Connecting to OpenSearch host: {host}")
        ...
        if host and username and password:
            ...
            logger.info("Using Secrets Manager for OpenSearch credentials")
        ...
        logger.info(f"Creating OpenSearch client for host: {host}:{port}")
        search_client = OpenSearch(hosts=hosts, http_auth=http_auth, use_ssl=True, verify_certs=False)

        # Test connection
        info = search_client.info()
        logger.info(f"OpenSearch connection successful. Version: {info.get('version', {}).get('number', 'unknown')}")
